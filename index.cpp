...
void TryTSP(int i, int h)
{ // try toi so khach hang xe h phuc vu
    for (int k = 0; k <= n; k++)
    { // thu tat ca khach hang
        if (pass[k] == false && x[k] == h)
        { //&&c[road[i-1]][k]!=0){
            pass[k] = true;
            road[i] = k;
            F = F + c[road[i - 1]][k];
...
int TSP(int i)
{ //tim quang duong ngan nhat cua xe i
    ...
    { // thu het cac xe
        ...
        { // neu con chat dc them
            ...
            if (i == n && minSothung() > 0)
            {
                ...
                for (int h = 1; h <= k; h++)
                { // tinh tong ngan nhat cua tung xe
                    int l = TSP(h);
                    f += l;
